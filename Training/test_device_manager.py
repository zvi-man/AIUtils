...
class TestDeviceManager(object):
    ...
    def test_locking_single_thread(self) -> None:
        for _ in range(self._NUM_OF_DEVICES_TO_ACQUIRE_SINGLE_THREAD):
            self.acquire_device()

    def test_locking_multiple_threads(self) -> None:
        threads = []
    ...
    def test_locking_timeout(self) -> None:
        try:
    ...
    def acquire_device(timeout_sec: int = _DEFAULT_TIME_OUT_SEC_DEVICE_MANAGER,
                       device_hold_time_sec: int = _TIME_FOR_DEVICE_HOLD) -> None:
